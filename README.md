**Целью итоговой работы по курсу "LLM Driven Development"** 

стала разработка двух ИИ-ассистентов, отличающихся функционалом и используемыми большими языковыми моделями.
...
Архитектура проекта представлена графически в виде блок-схемы ![](/docs/project_schema.png).
...
осуществляется последним этапом. После чего становится возможным отправка сообщений через Telegram-бот. 
По завершении сессии необходимо остановить работу сервера комбинацией клавиш CTRL + C.
...
Данные для ассистентов расположены в папке ./data и подразделяются на два типа:
    ...
    - Исходные данные: находятся в папке ./data/source_data и загружаются соответствующими 
      модулями при исполнении.
    - Предобработанные данные: формируются в ходе работы модулей и располагаются в папке 
      ./data/processed_data. 
...
Фрагмент запуска приложения из docker-контейнера приведен на скриншотах ![](docs/run-docker-image-1.png) и 
![](docs/run-docker-image-2.png), в соотвествующих файлах, расположенных в папке /docs
