...
talk_model_name = "sberbank-ai/rugpt3small_based_on_gpt2"
...
def ai_talk_answer(input_text):
    ...
    print(f'\nОтвет ассистента-собеседника:\n {output}\n')

    return # {"output": output}

# @observe()  
def ai_fin_answer(input_text):
    ...
    # Вывод в терминал
    print("\nОтвет ассистента-финконсультанта (подсазка - вопросы для меня можно посмотреть тут: ./data/processed_data/finai_data.json): ")
    print("Клиентский запрос: ", input_text)
    print('Ответ модели:\n', result)
    print(f'Время ответа модели: {execution_time:.2f}эn')

    return # {"output": result}  

def model_answer():
    user_request = input("Введите Ваш вопрос или текст: ")  # Получаем тело запроса
    # Получаем ответ от ассистента-собеседника
    ai_talk_answer(user_request)

    # Получаем ответ от ассистента-финконсультанта
    ai_fin_answer(user_request)
